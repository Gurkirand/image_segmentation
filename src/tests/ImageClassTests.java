...
public class ImageClassTests
{
	public static void main(String[] args)
	{
		...
		Function<BufferedImage, ImageMatrix> matrixF = (a) -> {return ImageProcessor.imageToMatrix(a, ImageProcessor.GRAYSCALE);};
		Function<Pair<String, ImageMatrix>, Boolean> saveF = (a) -> {return ImageProcessor.saveImage(a.first, a.second, ImageProcessor.GRAYSCALE);};
		...
		Timer.time("Save Grayscale Image Matrix", saveF, new Pair<>("output/tests/portrait_large_gray.jpg", imageMatrix), true);
		ImageMatrix blurMatrix = Timer.time("Gaussian Blur", blurF, new Pair<>(imageMatrix, 3));
		Timer.time("Save Blurred Image Matrix", saveF, new Pair<>("output/tests/portrait_large_blur.jpg", blurMatrix), true);
		...
		Timer.time("Save Segmented Image Matrix", saveF, new Pair<>("output/tests/portrait_large_segment.jpg", segment), true);
